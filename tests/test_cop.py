...
def test_compute_board_cop_zero_total_uses_board_center():
    result = compute_board_cop([0.0, 0.0, 0.0, 0.0], 0)
    assert result.valid is True
    assert result.cop_x == 0.0
    assert result.cop_y == 0.0

def test_compute_board_cop_at_or_below_one_kg_uses_board_center():
    result = compute_board_cop([0.25, 0.25, 0.25, 0.25], 0)
    assert result.valid is True
    assert result.total_force == 1.0
    assert result.cop_x == 0.0
    assert result.cop_y == 0.0
...
def test_compute_board_cop_with_correction_leaves_low_weight_at_center():
    result = compute_board_cop_with_correction([0.25, 0.25, 0.25, 0.25], 0, "board_1")
    assert result.valid is True
    assert result.total_force == 1.0
    assert result.cop_x == 0.0
    assert result.cop_y == 0.0
...
def test_compute_combined_cop_uses_midpoint_when_both_boards_are_low_weight():
    ...
    assert result.cop_x == pytest.approx(-28.75)
    assert result.cop_y == pytest.approx(0.0)
